# Reverse Image Location — Manual

English | [简体中文](MANUAL.zh-CN.md) · [← Back to the README](../README.md)

The complete reference: every way to run the app, every configuration knob, the full API, and the security model. The short front page lives in the [README](../README.md).
...
- **You still bring your own API key.** Nothing is bundled, no vendor is preselected, and nothing is billed to anyone else: start the app, click the gear at the top right, **choose a provider** — Claude (Anthropic) or an OpenAI-compatible gateway — then paste that provider's key, "Save & apply". Everything in [Where the API key comes from, and what a run costs](#where-the-api-key-comes-from-and-what-a-run-costs) applies unchanged.
...
**You use your own API key, pasted into the web page — no `.env` needed.** Gear icon at the top right → "Model" panel → choose a provider (Claude, or an OpenAI-compatible gateway) → paste that provider's API key → "Save & apply". It is stored **server-side** in `data/config.json` (gitignored) and survives browser switches and restarts. No provider is preselected: a fresh install reads "Not configured" until you choose one and add its key.

For the Claude provider, an Anthropic key comes from [console.anthropic.com](https://console.anthropic.com/) → API Keys. The account must have a **positive credit balance** — a key from an unfunded account authenticates fine and then fails at request time with a billing error.

**What one location run costs (Claude path)**: the preset model is Claude Opus 5, list-priced at $5 / million input tokens and $25 / million output tokens. A photo scaled down to 2000 px is about 4,000 input tokens; with the system prompt and schema the input totals about 8,000 tokens (≈ $0.04). The output is the thinking plus that JSON, usually a few thousand tokens. **It works out to roughly $0.1–0.2 per run**, and `MAX_TOKENS = 16000` is a hard ceiling, so even an absurdly long output caps at $0.40. If that's too slow or too expensive, first turn the **thinking effort** in the settings card from `medium` down to `low` — the single most direct knob in this app (see [Configuration](#configuration)).
...
The key still goes into the web page; `.env` is optional — if you want one, `cp .env.example .env` (changes to `.env` need a process restart; changes made in the web UI don't). The server starts fine with nothing configured: the startup log prints a warning, the chip at the top right reads "Not configured", and "Locate" returns `not_configured` until you choose a provider in the settings sheet and add its key.
...
- **`provider` is inference, never preference.** `provider` itself goes through the same four layers; if the merge yields `claude` / `openai` it is pinned (`provider_pinned = True`). Otherwise nothing is presumed: the server looks at which credential families the merged configuration actually holds. Exactly one family present — an Anthropic key, or a gateway whose `openai_base_url` and `openai_api_key` are **both non-empty** — selects that family; both families present leaves the server unconfigured until you choose in the settings sheet; neither family is unconfigured too. Whether credentials came from `data/config.json` or `.env` is fully equivalent: saving gateway credentials in the UI (with no Anthropic key around) infers `openai` exactly like writing them into `.env`.
...
| --- | --- | --- |
| `LLM_PROVIDER` | unset | `claude` / `openai`. Lowercased. Leave it unset and the server prefers nobody: it infers the provider from whichever credential family is present (see the inference rule below the table) |
| `ANTHROPIC_API_KEY` | empty | Official Anthropic API key. On the claude path, "configured" means this is non-empty |
| `ANTHROPIC_MODEL` | `claude-opus-5` | Claude model id — used (and shown) only once Claude is the resolved provider |
...
**The inference rule (no provider explicitly set)**: an Anthropic key as the only credential family selects `claude`; `OPENAI_BASE_URL` and `OPENAI_API_KEY` **both non-empty** as the only family selects `openai` (only the base_url without the key does not count); **both** families present → `configured: false` and the app asks you to choose a provider in the settings sheet; **neither** → `configured: false` as well. There is no built-in preference for any vendor.
...
Fields: a `PROVIDER` dropdown — a "— choose —" placeholder plus `Claude (Anthropic)` and `OpenAI-compatible gateway`; a fresh install preselects nothing — then Anthropic's API key / model name / **thinking effort**, and the OpenAI-compatible gateway's base URL / API key / model name.

The placeholder is not a vendor: leaving it selected expresses no opinion, and the server keeps resolving by inference. Neither dropdown offers a **"follow the server" option** either — a real deployment has no `.env`, and presenting a nonexistent file as a choice only misleads. Inputs can still be left empty — empty means inherit the next layer down, and if there's nothing there either, then there's nothing.
...
| `X-RIL-Provider` is the legacy value `auto` | Treated as unset — re-infer |
| `X-RIL-Provider` empty or absent | No opinion; the server-side layers below decide |
...
| Header absent, and no server layer names a provider | Nobody ever chose; infer |

"Infer" uses the same rule as startup: exactly one credential family present in the merged fields selects it (a gateway counts only when base URL and key are **both non-empty**); both families or neither resolves to nothing, and the request answers `not_configured`, asking you to choose a provider in the settings sheet. The last row must recompute rather than reuse startup's answer — the headers may have just completed a credential family for this one request. The sole reason `Settings.provider_pinned` exists is to tell these two kinds of "the server decides" apart. The web UI sends no `X-RIL-*` headers at all, so its requests always take the server-side resolution; direct calls via curl can hit every row.

> **Backwards compatibility**: the legacy value `auto` — what the old dropdown and old configs used to say — is still accepted anywhere a provider value is read (the `X-RIL-Provider` header, the stored `provider` in `data/config.json`, `LLM_PROVIDER` in `.env`) and is treated as unset, triggering the same inference. It no longer appears in any UI, placeholder, or example.
...
The frontend calls this at startup to render the model status at the top right. What's sent is **the server's currently effective configuration** (`data/config.json` over `.env`), not the header layer. `configured` false means the server could not resolve a usable provider — nobody chose one and the credentials on hand don't single one out, or the resolved provider's credentials are incomplete; `mapboxToken` is the public token verbatim, and the frontend uses the schematic globe when it's an empty string.
...
| --- | --- | --- |
| `ok` | true | Key works |
| `not_configured` | false | No provider could be resolved (none chosen, and the credentials present don't single one out), or that path's credentials are incomplete (Claude missing a key; gateway missing address or key) |
...
An HTTP error body is `{ "error": { "code": "...", "message": "..." } }` (inside an SSE `error` event it is the bare `{ "code", "message" }`), with a Chinese, user-presentable `message`. In English mode the web UI shows its own translation when it knows the `code`, and the server's message otherwise; direct API callers should switch on `code`.
...
| `bad_override` | 400 | An unusable `X-RIL-*` header: over 500 characters, non-ASCII or unprintable characters, provider not a recognized value, base URL not a complete http(s) address, or **a base-URL override without that gateway's key**. `PUT /api/settings` also returns it when **changing the gateway address without that gateway's key** | Fix the field named in `message`; returned by all four of `/api/locate`, `/api/locate/stream`, `/api/verify`, `PUT /api/settings` |
...
| `not_configured` | 503 | No usable provider: none was chosen and the credentials present don't single one out (none at all, or both families at once), or the resolved provider's credentials are incomplete | Open the settings sheet: choose a provider and add its key ("Save & apply", effective immediately), or set `.env` and restart; the openai path needs base_url and key both |
...
│  ├─ config.py        environment variables → frozen Settings dataclass, provider inference rule
...
│  └─ MANUAL.zh-CN.md  its Chinese edition
