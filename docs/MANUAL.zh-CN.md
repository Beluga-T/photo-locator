...
[English](MANUAL.md) | 简体中文 · [← 返回 README](../README.zh-CN.md)

完整参考：每一种启动方式、每一个配置项、完整的 API 与安全模型。简短的首页见 [README](../README.zh-CN.md)。
...
- **API Key 仍然要你自己的。** 程序里没有内置任何密钥，不预选任何一家模型，也不会去花别人的钱：启动之后点右上角齿轮，**先选一个模型来源**——Claude（Anthropic）或 OpenAI 兼容网关——再把那一家的 Key 粘进去，「保存并生效」。「API Key 从哪来、一次多少钱」那一节原封不动地适用。
...
**API Key 用你自己的，填在网页里，不需要建 `.env`。** 页面右上角齿轮 →「模型」面板 → 先选模型来源（Claude / Anthropic，或 OpenAI 兼容网关）→ 把那一家的 API Key 粘进去 →「保存并生效」。它存在**服务端**的 `data/config.json`（已 gitignore），换浏览器、重启程序都还在。全新安装不预选任何一家：在你选定来源并填好密钥之前，右上角一直显示「未配置」。

选 Claude 的话，密钥在 [console.anthropic.com](https://console.anthropic.com/) → API Keys 里创建；账户要有**正余额**——没充值的账户，密钥能通过鉴权，到真正分析时才报计费错误。

**一次定位大概多少钱（Claude 通道）**：预置模型是 Claude Opus 5，官方价格是输入 $5 / 百万 token、输出 $25 / 百万 token。一张缩到 2000 px 的照片约 4000 个输入 token，加上系统提示词与 schema，输入合计约 8000 token（≈ $0.04）；输出是思考加那份 JSON，通常几千 token。**估下来一次 0.1–0.2 美元左右**，`MAX_TOKENS = 16000` 是硬上限，所以单次输出再离谱也就 $0.40。嫌贵或嫌慢，先把设置卡里的**思考深度**从 `medium` 调到 `low`，那是这个应用里最直接的一个旋钮（见「配置」）。
...
密钥仍然是在网页里填的；`.env` 是可选的，想用就 `cp .env.example .env`（改完 `.env` 要重启进程才生效，改网页里的配置不用）。什么都没配也能起服务，启动日志会打一条 warning，前端右上角芯片显示「未配置」，点「开始定位」会拿到 `not_configured`——直到你在设置卡里选好模型来源并填上那一家的密钥。
...
- **provider 靠推断，不靠偏好。** `provider` 本身也走这四层；合并后如果是 `claude` / `openai`，那就是钉死的（`provider_pinned = True`）。否则不预设任何一家：服务端看合并后的配置里实际有哪些凭据族——只有一族（一把 Anthropic key，或 `openai_base_url` 与 `openai_api_key` **两者都非空**的网关）就选那一族；两族都在就保持未配置，等你在设置卡里亲自选；一族都没有同样是未配置。凭据来自 `data/config.json` 还是 `.env` 完全等价：在界面里存一套网关凭据（且没有 Anthropic key），和写进 `.env` 一样会被推断成 `openai`。
...
| 变量 | 默认值 | 含义 |
| --- | --- | --- |
| `LLM_PROVIDER` | 空 | `claude` / `openai`。取值会被 lowercase。留空则服务端不偏向任何一家，按现有凭据族推断（见表下的推断规则） |
| `ANTHROPIC_API_KEY` | 空 | Anthropic 官方 API Key。走 claude 时它非空才算「已配置」 |
| `ANTHROPIC_MODEL` | `claude-opus-5` | Claude 模型 id——只有在 Claude 被选定或被推断为模型来源后才会被使用和展示 |
...
**推断规则（没有明确指定 provider 时）**：只有一把 Anthropic key 这一族凭据就选 `claude`；只有 `OPENAI_BASE_URL` 和 `OPENAI_API_KEY` **两者都非空**的网关这一族就选 `openai`（只填 base_url 没填 key 不算数）；**两族都在**则 `configured: false`，应用会提示你在设置卡里选择模型来源；**一族都没有**同样是 `configured: false`。不存在偏向任何一家的内置默认。
...
字段：`PROVIDER` 下拉——占位项「— 请选择 —」加 `Claude (Anthropic)` 与 `OpenAI 兼容网关`，全新安装不预选任何一家——以及 Anthropic 的 API Key / 模型名 / **思考深度**、OpenAI 兼容网关的 Base URL / API Key / 模型名。

占位项不是一家厂商：停在占位项上表示「没有意见」，服务端继续按凭据推断。两个下拉也都**不提供「跟随服务端」这一项**——真实部署里没有 `.env`，把一个不存在的文件摆成选项只会误导。输入框仍然可以留空——留空表示沿用下一层的值，下一层也没有就是没有。
...
| `X-RIL-Provider` 是下面那个旧值 `auto` | 当作「未指定」，重新推断 |
| `X-RIL-Provider` 为空或没发这个头 | 没有意见；由下面几层服务端配置决定 |
| 没发这个头，且服务端的 `provider` 明写了 `claude` / `openai`（`data/config.json` 或 `LLM_PROVIDER`） | 操作者钉死的，照办 |
| 没发这个头，且服务端哪一层都没写明 | 谁都没选过，重新推断 |

「重新推断」用的还是启动时那条规则：合并后的字段里只有一族凭据就选那一族（网关要 base URL 与 key **两者都非空**才算一族）；两族都在或一族都没有就是解析不出来，这次请求返回 `not_configured`，提示你在设置卡里选择模型来源。最后一行必须重算而不能沿用启动时的答案——请求头可能刚好为这一次请求补齐了一族凭据。`Settings.provider_pinned` 存在的唯一意义就是区分这两种「服务端说了算」。网页前端根本不发 `X-RIL-*` 头，所以它的请求永远走服务端解析；curl 之类的直接调用才会命中每一行。

> **向后兼容**：旧值 `auto`——旧版下拉和旧配置写的那个词——在所有读 provider 的地方（`X-RIL-Provider` 请求头、`data/config.json` 里存的 `provider`、`.env` 的 `LLM_PROVIDER`）仍被接受，含义等同于「未指定」，走同一套推断。它不再出现在任何界面、占位符或示例里。
...
前端启动时调它来渲染右上角的模型状态。这里下发的是**服务端当前生效的那份配置**（`data/config.json` 压过 `.env`），不含请求头那一层。`configured` 为 false 表示服务端解析不出一个可用的模型来源——没人选过而现有凭据又推断不出唯一一家，或解析出来那家的密钥没填齐；`mapboxToken` 是那个公开令牌的原样下发，为空串时前端改用示意地球。
...
| --- | --- | --- |
| `ok` | true | 密钥可用 |
| `not_configured` | false | 解析不出模型来源（没人选过，现有凭据也推断不出唯一一家），或该通道的凭据没填齐（Claude 缺 key，网关缺地址或 key） |
...
| `bad_override` | 400 | 某个 `X-RIL-*` 头不可用：超过 500 字符、含非 ASCII 或不可打印字符、provider 不是可识别的取值、Base URL 不是 http(s) 完整地址，或**覆盖了 Base URL 却没同时给该网关的 Key**。`PUT /api/settings` 在**改网关地址却没同时给该网关 Key** 时也返回它 | 按 `message` 改对应字段；`/api/locate`、`/api/locate/stream`、`/api/verify`、`PUT /api/settings` 四处都会返回它 |
...
| `not_configured` | 503 | 解析不出可用的模型来源：没人选过而凭据推断不出唯一一家（一族都没有，或两族同时在场），或解析出来那家的密钥没填齐 | 打开设置卡：选择模型来源并填上那一家的密钥（「保存并生效」立即生效），或填 `.env` 后重启；openai 通道需 base_url 与 key 同时具备 |
...
├─ app/
│  ├─ __init__.py      版本号
│  ├─ config.py        环境变量 → 冻结的 Settings 数据类，provider 推断规则
