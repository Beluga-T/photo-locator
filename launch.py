...
def banner(url: str, data_root, requested_port: int, port: int, portable: bool) -> None:
    ...
    say("")
    say("  第一次使用：打开页面右上角的设置，选择一家模型来源")
    say("  （Claude 或 OpenAI 兼容网关），填入你自己的 API key。")
    say("  它只保存在上面那个文件夹里，不会传到别处。")
    say("")
    say("  First run: open the settings sheet at the top right, choose a")
    say("  provider (Claude or an OpenAI-compatible gateway), and paste your")
    say("  own API key. It stays in the folder above.")
    say("")
